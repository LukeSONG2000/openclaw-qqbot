...
assert.equal(alert.text?.includes("窗口：15分钟 内 3 次"), true);
assert.equal(alert.text?.includes("响应超时（response-timeout）=2"), true);
assert.equal(alert.text?.includes("上下文过长（context-too-long）=1"), true);
assert.equal(alert.text?.includes("待处理=5, 活跃=1/2, 当前会话待处理=2, 活跃时长=2分钟/3分钟"), true);
