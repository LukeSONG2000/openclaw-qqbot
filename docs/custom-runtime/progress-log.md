...
- `/bot-fallback list` now expands urgent queue-bypass records with command, dropped count, queue peer id, and after-clear pending counts; `/bot-fallback summary` counts them separately.
