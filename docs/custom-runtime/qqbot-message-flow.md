...
- Binding values must be QQBot platform openids, not raw QQ numbers: `customRuntime.admins` uses `user_openid` or `member_openid`, while `customRuntime.adminGroup` uses `group_openid` / `qqbot:group:<group_openid>`. If only human QQ `1137586795` or group `945739251` is known, the safe path is a challenge message sent in that C2C/group so the gateway can read the openid fields from the inbound event.
